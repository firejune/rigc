...
This is rigc and the benchmark ladder as they were measured on 2026-08-22: what the tool
emitted and checked on that date, what Spine's official example projects actually use, and
the gap list the ladder was climbed by. It is not kept current — what rigc does now is in
[AUTHORING.md](AUTHORING.md), and where the ladder stands is in [LADDER.md](LADDER.md). The
Spine 4.3 format reference this survey once contained — the format-change timeline and the
Part 1 that the sections below cite as §1.N — is [SPEC_COVERAGE.md](SPEC_COVERAGE.md).
