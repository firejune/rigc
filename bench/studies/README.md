...
| [`2026-09-05-density`](2026-09-05-density/README.md) | Do the generators, `A39` and the DEFORM report hold at thousands of vertices, and what does density cost? ([#381](https://github.com/firejune/rigc/issues/381) stage 2) | **They hold** — linear in triangles to 64,800, zero failures, and the wall is the artifact at 449 bytes a vertex (14.7 MB per part, +1.77 MB per key). But the fold angle turns out to belong to the **depth map's steepest slope**, not to the mesh: a dome falls 62° → 14° over a 1,300× refinement while a bounded map holds 63–64°. `contour` is not a dense generator — it saturates at 868 vertices and reads 2 % of the map |
| [`2026-09-05-noise`](2026-09-05-noise/README.md) | Does a depth map's grain set the turn ceiling instead of its shape, and at what amplitude? | **It does, from ±1 LSB up** — a one-level perturbation costs 3.2° of a 64.77° ceiling at 4,225 vertices and one stray pixel of 160,000 costs 58°, with `A39` refusing at each reported angle (gap 0°). Density makes all of it worse. Two closed forms bound it: `ceiling ≤ atan(255·h/zScale)`, exact in nine rows, and a texel-step floor of `atan(255/(zScale·D))`, hit to two decimals at 641,601 vertices. Recommends a diagnostic (`p1/min` = 1.003 clean against 10.652 for a stray pixel) and explicitly **not** a filter |
| [`2026-09-06-readers`](2026-09-06-readers/README.md) | What set of values can each of the twelve slider readers actually produce? ([#420](https://github.com/firejune/rigc/issues/420) stage 1) | **Six ends, measured, and one correction.** `scaleX`/`scaleY` world are `[0, ∞)` with **0 attained** by a degenerate bone and the sign lost to `Math.sqrt`; `shearY` world is `[−449.99999468, 269.99999468]`, both ends hit exactly, but its real hazard is a 360°-wide window whose seam moves with the bone's orientation; `x`/`y` are unbounded in both branches and a negative skeleton scale changes **nothing** (rejected candidate). 🚨 `rotate` world is **`[0, 360]`**, not `[0, 360)` — the wrap rounds — and `local: true` is a passthrough only on a bone no constraint drives |
