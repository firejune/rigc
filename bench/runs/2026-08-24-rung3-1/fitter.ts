...
// We know from setup analysis:
// Pendulum pivot is at: (199.0, 707.0)
// Pendulum attachment offset is: (-307.5, 0)
// Square setup is at: (381.35, 78.6)
const PIVOT_X = 199.0;
const PIVOT_Y = 707.0;
const ATT_X = -307.5;
const ATT_Y = 0;
...
function fitFrame(ref: Plate, initPRot: number, initSX: number, initSY: number, initSRot: number, squareCanMove: boolean): FramePose {
  ...
  // 1. Broad pendulum scan [-160 .. 20]
  for (let rot = -160; rot <= 20; rot += 2) {
  ...
  if (squareCanMove) {
    ...
    // World x ranges from 300 to 1600, y ranges from 50 to 500
    const xMin = Math.max(300, bestSX - 200);
    const xMax = Math.min(1600, bestSX + 200);
    const yMin = Math.max(50, bestSY - 150);
    const yMax = Math.min(600, bestSY + 150);

    for (let sx = xMin; sx <= xMax; sx += 20) {
      for (let sy = yMin; sy <= yMax; sy += 20) {
    ...
    for (let dsx = -25; dsx <= 25; dsx += 2) {
      for (let dsy = -25; dsy <= 25; dsy += 2) {
...
// Heavy: impact at frame 9, rest at frame 30
...
// Light: impact at frame 4, rest at frame 6
