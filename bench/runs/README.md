...
| spineboy | [`spineboy.md`](../briefs/spineboy.md) | **yes (third party) ×3**, rev 4, 2026-08-27 — the revision-2 gun-hand contradiction ([#111](https://github.com/firejune/rigc/issues/111)) is **settled by measurement** (rev 3): the gun is in the **far** hand, `idle` and `walk` say so, and nothing about the gun's arm is left for a run to rediscover. Rev 4 is the first pass under the tolerance-in-claim rule and the fifth reading, and it found the brief counting differences at **2/255**, finer than the measure — so `ess/death`'s nine-frame hold read "24–45 px" where the per-frame comparison reads **eight exact holds and one 1 px pair**, and the shot's f13–f17 tail (the feet coming to rest after the body has) was described by no passage at all. Both are fixed, `run` gained the blaster's 121 px sweep, and the pointer at this repository's ladder row is gone | 2 skeletons, 19 animations, 12 fps in full **+ 30 fps sheets** | **7** (`ess`; `pro` is optional) | the rung clears on **`ess` alone**, and `ess` has **8** animations — the ladder row's "11" is `pro`'s count; `pro`'s line is printed *(stretch — does not count)*; the two skeletons are at different scales, so no pixel figure crosses between them; the second rate is **30 fps** and only two frames per animation are on disk at it, so the sheets are for timing and never for a distance. ⚠️ **This brief's difference counts are at 2/255 throughout** — read every "still" at `check`'s 8, which the brief now does for you |
| 7 | [`7-anticipation.md`](../briefs/7-anticipation.md) | **yes (third party)**, rev 3, 2026-08-28 — rev 3 promotes two claims out of their hedges after the first climb: the panel's draw-order edge is **frames-forced** (flanked-beige measurement on all 102 frames), and the sack **needs a deforming attachment** (no affine image of the art fits any frame). Both re-derived from local frames, neither taken from the run's record. The cape's mechanism is still undecidable and the brief now says the sack's verdict must not be read across to it | **local-only, never committed**: 4 animations, 102 frames at 12 fps, plus 24 and 30 fps sheets | **3** | the frames are not in this repository: render them yourself with the exact commands in the brief, into the gitignored `bench/reference-local/`, and pass `--frames` to `check` and `bench`. The brief's figures depend on conventions it states and none of which is the obvious choice — opaque means **alpha ≥ 128** on the art, velocity is a **central difference**, "differ in colour" counts are at **exact equality** while silhouettes are at 8/255, the flanked-beige test runs on **raw** masks, and the affine residual is quoted across **three grid resolutions** because a control at the art's own density measures point count rather than shape. 🆕 **The `scale: 0.5` texture floor IS measurable here as of 2026-08-29 — with `--texture-from`, never `--atlas`** ([PR #254](https://github.com/firejune/rigc/pull/254), [#171](https://github.com/firejune/rigc/issues/171)/[#199](https://github.com/firejune/rigc/issues/199)). This atlas packs its regions `rotate: 270` and trimmed, so `check --atlas` re-seats region **geometry** as well as texture and sends the figure the *wrong* way (up, on every set) — that is why the two attempts read it as inconclusive and why attempt 1's floor figure must still not be re-quoted. `--texture-from <the example's own atlas>` keeps every world vertex and swaps only the texels: the floor measures **1.45–1.85 MAE(ref)** across the twelve sets and the texture explains **1.3–2.6 %** of each figure, so the error here is the rig. 🆕 **And the declared box is now *taken* on ten of the twelve sets** (`extent-spread`, [#194](https://github.com/firejune/rigc/issues/194)) rather than refused on every one — `walk@24fps` and `walk@30fps` are still refused, on `coordinates`. ⇒ Expect the frames' own box on most sets, and expect figures that do **not** compare with either attempt's recorded ones: the same stored candidate reads worst drift **3.94 px** where the record says 2.76. ✅ **Rung 7 is cleared** — on **attempt 3** under **gate v2.3**, 2026-09-02 (LADDER's *gate-v2.3 re-inspection*). 🗓️ Its history in one line, because three of its verdicts are on the same rung within six days: attempt 2 **passed** gate v2.1/v2.2, that pass was **withdrawn** on 2026-09-02 under the [#254](https://github.com/firejune/rigc/pull/254) instrument, attempt 3 **FAILed gate v2.2** the same day on the read-down enumeration, and it **passes gate v2.3** on the clause decisions that FAIL filed. ⇒ **Read those sections before quoting any figure from its three runs** — the figures do not compare across the instrument change. 🆕 **What attempt 3 settled, so a fourth does not re-spend it**: `cape-back`'s attribution is a **ceiling of the shot**, not a fit — the crimson the sack is not in front of, over the smallest oriented rectangle covering it, is **5–19 %** on all 102 frames against a **~66 %** floor below which nothing in this corpus is attributed, so no placement, scale, rotation, pivot or region-versus-mesh choice makes that slot attributable at the declared box. That measurement is now **the read-down itself**, under v2.3's fifth kind, paired with a placement pin of **≤ 2–3 px** and build-side draw order. 🚫 And two shapes that *would* clear the matcher stay **out of bounds**, both refused with reasons on attempt 3: a U-shaped or annular hull hugging the visible rim (the reading rev 3 says the frames do not support) and keying the slot empty where the panel is occluded (which silences the limb about a part that is there) |
