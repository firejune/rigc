 ...
 * `A19`'s alpha scan below, the region lift `partPlate` traces a mesh generator
 * over in [`src/compile.ts`](compile.ts), and `spine-html`'s region tier, which
  ...
  check('A06_ATLAS_PAGE_SIZE_MATCHES_PNG', () => {
    ...
    for (const page of atlas.pages) {
      ...
      if (info !== null && (page.width !== info.width || page.height !== info.height)) {
        ...
        // both profiles. See `pageGridReading` for the routines and the figures.
        const grid = pageGridReading(page, info);
        const said = `page "${page.name}" declares ${page.width}x${page.height} and its PNG is ${info.width}x${info.height}`;
        if (grid === null) {
          fail('A06_ATLAS_PAGE_SIZE_MATCHES_PNG', `${said}. ${PAGE_GRID_PREAMBLE}`);
        } else {
          const ratios =
            `${said} — ${gridRatio(grid.x)} of the declared width and ${gridRatio(grid.y)} of the declared ` +
            'height.';
          const onPage = atlas.regions.filter((region) => region.page.name === page.name);
          const off = grid.uniform ? firstNumberOffTheCoarseGrid(onPage, page.width, info.width) : null;
          const repair = !grid.uniform
            ...
            : off !== null
              ...
              : 'The format states coarser texels with the `scale:` header and rigc builds that: declare ' +
                `\`size: ${info.width}, ${info.height}\` with \`scale: ${gridRatio(grid.x)}\` and multiply every ` +
                `\`bounds\`/\`offsets\` on this page by ${gridRatio(grid.x)}, and every part keeps the size it ` +
                'has now.';
          fail('A06_ATLAS_PAGE_SIZE_MATCHES_PNG', `${ratios} ${PAGE_GRID_PREAMBLE} ${repair}`);
        }
      }
